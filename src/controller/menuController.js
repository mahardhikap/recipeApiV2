const {postMenu, getMenuAll, getMenuById, getSearchSortMenu, putMenuById, getMenuByUser, getSearchSortMenuByUser, delMenuById} = require('../model/menuModel')
...
const menuController = {
    addMenu: async (req, res) => {
        try {
            ...
            if (result.rows[0]) {
                return res.status(200).json({status:200, message: 'Post menu success!', data:result.rows[0]})
            }
        } catch (error) {
            console.error('Post menu error', error.message)
            return res.status(500).json({status:500, message: 'Post menu failed!'})
        }
    }, 
    getMenu: async (req, res) => {
        try {
            ...
            if(result.rowCount > 0){
                return res.status(200).json({status:200, message:'Get menu success!', data:result.rows})
            } else {
    ...
    getMenuId: async (req, res) => {
        try {
            ...
            if(result.rows[0]){
                return res.status(200).json({status:200, message:'Get menu by id success!', data:result.rows[0]})
            } else {
                return res.status(404).json({status:404, message:'Data not found!'})
            }
        } catch (error) {
            console.error('Error when get menu by id', error.message)
            return res.status(500).json({status:500, message:'Get menu by id failed!'})
        }
    },
    searchSortMenu: async (req, res) => {
        try {
            ...
            if (result.rows.length > 0) {
                return res.status(200).json({status:200, message:'Get data success!', data:{rows:result.rows, pages:pagination}});
            } else {
                return res.status(404).json({status:404, message:'Data not found!'});
            }
        } catch (error) {
    ...
    editMenu: async (req, res) => {
        try {
            ...
            if(user_id !== data.rows[0].user_id){
                return res.status(404).json({status:404, message:'This is not your post!'})
            }

            const result = await putMenuById(post)
            if(result.rows[0]){
                return res.status(200).json({status:200, message:'Edit menu success!', data:result.rows[0]})
            }
        } catch (error) {
            console.error('Error when update menu', error.message)
            return res.status(500).json({status:500, message:'Update menu failed!'})
        }
    },
    ...
    deleteMenu: async (req, res) => {
        try {
            ...
            if(result.rows[0]){
                return res.status(200).json({status:200, message:'Delete menu success!', data:result.rows[0]})
            } else {
                return res.status(404).json({status:404, message:'Data not found!'})
            }
        } catch (error) {
            console.error('Error when delete menu', error.message)
            return res.status(500).json({status:500, message:'Delete menu failed!'})
        }
    }
}
