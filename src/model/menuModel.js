...
const getMenuById = async (id) => {
    return new Promise((resolve, reject)=>{
        ...
        pool.query(`SELECT recipe.id, recipe.title, recipe.photo AS photo_menu, recipe.ingredients, category.name AS category, register_user.username, register_user.photo AS photo_user, recipe.created_at FROM recipe JOIN category ON recipe.category_id = category.id JOIN register_user ON recipe.user_id = register_user.id WHERE id = ${id}`, (err, results) => {
            if (!err) {
                resolve(results)
            } else {
                reject(err)
